...
jQuery(document).ready(function() {
	...
	$('body').on('click', '#reset', function(event) {
		...
		$(this).closest('#example').find('#finish-number').val(30);

		var fizzBuzzCount = function(where) {
			$(where).html(0);
		};

		fizzBuzzCount('#fizz-data');
		fizzBuzzCount('#buzz-data');
		fizzBuzzCount('#fizzbuzz-data');
	...
	$('body').on('click', '#start', function(event) {
		...
		if (isNaN(endNum) && isNaN(startNum)) {
			alert(startNum+" and "+endNum+" are not numbers. Please enter numbers.");
		} else if (isNaN(endNum)) {
			alert(endNum+" is not a number. Please enter a number.");
		} else if (isNaN(startNum))  {			
			alert(startNum+" is not a number. Please enter a number.");
		} else {
			startNum = +startNum;
			endNum = +endNum;
